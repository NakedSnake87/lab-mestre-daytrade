...
6. Ao analisar gráficos: descreva tendência, médias visíveis, suportes e resistências. Não invente padrões.
7. Linguagem de trader veterano — sem academicismo.
8. Sobre tendência do dólar/índice hoje: use os dados de mercado fornecidos no contexto e analise com base neles. Se não houver dados suficientes, oriente o trader a verificar o gráfico e explique o que observar."""
...
def buscar_cotacoes():
    ...
    try:
        session.get("https://finance.yahoo.com", timeout=6)
    except:
        pass
    crumb = ""
    try:
        rc = session.get("https://query2.finance.yahoo.com/v1/test/getcrumb", timeout=6)
        if rc.status_code == 200:
            crumb = rc.text.strip()
    except:
        pass
    tickers_str = "%2C".join(ATIVOS.values())
...
# WIN: cada ponto = R$ 0,20 × lote-padrão de 5 → R$ 1,00/ponto por contrato  (convenção B3)
# WDO: cada ponto = R$ 10,00 por contrato  (1 contrato = US$10.000, variação em R$/US$)
# Referência B3: WIN ponto mínimo 5 pts = R$ 5,00; WDO ponto mínimo 0,5 pt = R$ 5,00
# Usaremos: WIN = R$ 1,00/ponto   |   WDO = R$ 10,00/ponto
MULT = {"WIN": 1.0, "WDO": 10.0}
...
.noticia-card { background:#0f172a; border:1px solid #1e293b; border-radius:12px; padding:1rem 1.2rem; margin-bottom:.75rem; transition:all .2s; }
...
.noticia-desc   { font-size:.82rem; color:#94a3b8; margin-top:.4rem; line-height:1.5; }
...
.chat-container { max-height:420px; overflow-y:auto; padding:.5rem; scrollbar-width:thin; scrollbar-color:#1e293b transparent; }
...
with tab1:
    ...
    with col_btn:
        if st.button("⟳  Atualizar Mercados"):
            ...
            st.rerun()
    with col_info:
        st.markdown("<div style='color:#475569;font-size:.78rem;padding-top:.6rem'>Atualização automática a cada 60s · Dados: Yahoo Finance</div>", unsafe_allow_html=True)
    ...
    def card_html(nome, dados):
        ...
        var   = dados.get("var",   0)
        if not preco:
            return f'<div class="ativo-card"><div class="ativo-nome">{nome}</div><div class="ativo-preco" style="color:#475569">—</div><div class="ativo-var-nt">sem dado</div></div>'
        if   preco > 10000: ps = f"{preco:,.0f}".replace(",",".")
        elif preco > 100:   ps = f"{preco:,.2f}".replace(",","X").replace(".","," ).replace("X",".")
        elif preco > 1:     ps = f"{preco:.4f}"
        else:               ps = f"{preco:.6f}"
    ...
    st.markdown('<div class="sec-divider"></div><div class="sec-title">📰 Notícias do Mercado</div>', unsafe_allow_html=True)
    ...
    if not noticias:
        st.markdown('<div style="color:#475569;font-size:.85rem;padding:1rem 0">Nenhuma notícia encontrada.</div>', unsafe_allow_html=True)
    else:
        for n in noticias[:6]:
            titulo = n.get("title","")
            ...
            <div class="noticia-card">
              ...
              <div class="noticia-desc">{desc[:180]+('...' if len(desc)>180 else '')}</div>
...
with tab2:
    ...
    with col1:
        ...
        # % de risco sugerido automático
        pct_max    = risco_sugerido(capital)
        pct_padrao = min(pct_max, 2.0)
        st.markdown(f'<div class="risco-sugerido">💡 Para R$ {capital:,.0f} o risco sugerido é até <b>{pct_max:.0f}%</b> por operação.</div>', unsafe_allow_html=True)
        risco_pct  = st.number_input("% do capital a arriscar (máx 10%)", min_value=0.5, max_value=10.0, value=pct_padrao, step=0.5)
        if risco_pct > pct_max:
            st.markdown(f'<div class="calc-alerta">⚠️ Você está acima do limite sugerido de {pct_max:.0f}% para este capital.</div>', unsafe_allow_html=True)
    ...
    # ── Valores por ponto (B3 oficial) ────────────────────────────────────────
    # WIN: R$ 1,00 por ponto por contrato  (lote padrão: 5 contratos mín, mas cálculo unitário = R$1/pt)
    ...
    if st.button("📊  Calcular Risco"):
        ...
        <div class="calc-result">
          ...
          <div class="calc-linha"><span class="calc-label">Valor por ponto</span><span class="calc-valor">R$ {val_ponto:.2f}/pt · {n_contratos} contrato(s)</span></div>
          <div class="calc-linha"><span class="calc-label">Perda máxima (stop)</span><span class="calc-valor" style="color:{risco_cor}">R$ {perda_pts:,.2f}</span></div>
          <div class="calc-linha"><span class="calc-label">Ganho potencial (meta)</span><span class="calc-valor" style="color:#22c55e">R$ {ganho_pts:,.2f}</span></div>
          ...
          <div class="calc-linha"><span class="calc-label">Limite permitido ({risco_pct}%)</span><span class="calc-valor">R$ {risco_real:,.2f}</span></div>
        ...
        if stops_ate_zerar <= 5:
            st.markdown(f'<div class="calc-alerta">🚨 Apenas {stops_ate_zerar} stops seguidos zeram sua conta. Reduza o tamanho.</div>', unsafe_allow_html=True)
...
with tab3:
    ...
    with col_chat:
        ...
        if not st.session_state.historico:
            chat_html += '<div style="color:#475569;font-size:.85rem;padding:1rem 0;text-align:center">👋 Pode perguntar sobre WIN, WDO, indicadores ou mandar print do gráfico.</div>'
        else:
            for msg in st.session_state.historico[-20:]:
                c = msg["content"].replace("<","&lt;").replace(">","&gt;")
                if msg["role"] == "user":
        ...
        with col_r:
            if st.session_state.historico:
                st.markdown(f'<div style="font-size:.72rem;color:#475569;padding-top:.6rem;text-align:right">{len(st.session_state.historico)//2} mensagem(s)</div>', unsafe_allow_html=True)
